package com.khidmasoft.xamsambokk.web.rest;
...
import com.khidmasoft.xamsambokk.config.KafkaProperties;
...
class XamsambokkKafkaResourceIT {
    ...
    void setup() {
        ...
        XamsambokkKafkaResource kafkaResource = new XamsambokkKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/xamsambokk-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/xamsambokk-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
